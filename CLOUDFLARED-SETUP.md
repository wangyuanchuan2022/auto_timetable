...
> - **进程活但隧道断线**（2026-09-10 实证：公网侧 TLS 握手失败/无边缘连接）：v2 守护的僵尸探测会在下一循环自动 taskkill 重拉（无需人工）；若守护本身也不在了，`Stop-Process -Name cloudflared -Force` 后运行 `node .qrtest\spawn-via-worktable.mjs "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\start-cloudflared.cmd"`（经宿主通道拉起 Startup shim，等价开机自启链路；直接 Start-Process 会被沙箱 job-object 回收）；新 URL 约 10 秒内写入 tunnel.log 并被服务端自动发现，**地址会变，APP/手机书签需重新扫码**
