...
- **图片附件原生解码（v1.6）**：WebView 的 canvas 无法解码 HEIC/HEIF（Chromium 限制），新手机相册常见格式会被静默丢弃——现由壳用 Android BitmapFactory 解码（`NativeBridge.compressImage`，等比缩到 1568px、JPEG 82%）后再发；页面对**任何**未加入的文件（非图片 / 原生也解不了）都会给出明确提示，绝不静默丢图。纯浏览器环境仍走 canvas 路线（行为不变；页面逻辑由服务端托管，刷新页面即生效）。
