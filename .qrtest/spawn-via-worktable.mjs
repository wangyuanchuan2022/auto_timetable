...
// 2026-09-11 参数化：argv[2] 可选传一个 .cmd/.脚本的绝对路径 → 隐藏窗口启动它（守护进程等
// 长驻任务复用）；不传 = 维持原行为拉起 mobile-server。
import { worktableToken, termUrl } from './worktable-auth.mjs';

const target = process.argv[2] || null;
const CMD = target
  ? `Start-Process cmd -ArgumentList '/c','"${target}"' -WindowStyle Hidden`
  : "Start-Process node -ArgumentList 'mobile-server.mjs' -WorkingDirectory 'D:\\tools\\auto_timetable' -WindowStyle Hidden";
