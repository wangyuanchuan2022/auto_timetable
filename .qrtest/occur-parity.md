...
- 用例来源：`.qrtest/occur-test.mjs` 的 occursOn 表（50 例：weekly 基本/边界/deadline、once、custom×day|week|month×interval×days×until×deadline×跨午夜组合、skip 例外日期、weekPattern 单双周、skip+weekPattern 组合、task 长周期任务截止日）。
...
## 结果（2026-09-02，task 长周期任务更新）

```
parity: 50 same, 0 diff, total 50
```

50 例全部一致，零分歧（P2-3 时为 46 例全同，本次新增 task 4 例）。覆盖的关键语义点均对齐：

| 语义点 | 双侧一致行为 |
| --- | --- |
| deadline | 到该日（含）为止生效；次日不再发生 |
| task | 长周期任务：仅截止当日「发生」（deadline = 必须完成日）；前后日均不发生；缺 deadline 永不发生 |
